...
async function MenuReportsRebalancingGo() {
    console.log('🔧 MenuReportsRebalancingGo() called');

    try {
        console.log('🔧 Step 1: Initializing grid...');
        ...
        console.log('🔧 Step 2: Initializing asset class totals...');
        for (let assetClass in AssetClassConfig) {
            assetClassTotals[assetClass] = 0;
        }
        console.log('✅ Asset classes initialized:', Object.keys(assetClassTotals));

        // Fetch account and portfolio data
        console.log('🔧 Step 3: Fetching account data...');
        let snapshotData = await getAccountsData();
        console.log('✅ Account data fetched, accounts count:', snapshotData?.accounts?.length || 0);

        console.log('🔧 Step 4: Fetching portfolio data...');
        let lowerDate = formatQueryDate(getDates('d_Today'));
        let higherDate = formatQueryDate(getDates('d_Today'));
        console.log('🔧 Date range:', lowerDate, '-', higherDate);

        let portfolioData = await getPortfolio(lowerDate, higherDate);
        console.log('✅ Portfolio data fetched, holdings count:', portfolioData?.portfolio?.aggregateHoldings?.edges?.length || 0);

        // Step 1: Build account holdings data with inferred cash
        console.log('🔧 Step 5: Building account holdings...');
        await BuildAccountHoldings();
        console.log('✅ Account holdings built, total portfolio:', totalPortfolio, 'total cash:', totalCash);

        // Step 2: Build asset allocation table
        console.log('🔧 Step 6: Building asset allocation table...');
        await BuildAssetAllocationTable();
        console.log('✅ Asset allocation table built');

        // Step 3: Build account summary cards
        console.log('🔧 Step 7: Building summary cards...');
        await BuildSummaryCards();
        console.log('✅ Summary cards built');

        // Step 4: Build rebalancing recommendations
        console.log('🔧 Step 8: Building recommendations...');
        await BuildRebalancingRecommendations();
        console.log('✅ Recommendations built');

        glo.spawnProcess = 1;
        console.log('🎉 MenuReportsRebalancingGo() completed successfully!');

    } catch (error) {
        console.error('❌ ERROR in MenuReportsRebalancingGo():', error);
        console.error('❌ Error stack:', error.stack);
        throw error; // Re-throw to see it in the UI
    }
