...
function MF_DrawChart(inLocation) {
    ...
    if(inLocation != null) {
        ...
        divTooltip = cec('div','',divTop,'','','position: absolute; background: #000000; color: #fff; padding: 12px; border-radius: 6px; pointer-events: none; font-size: 14px; font-weight: 600; display: none;','id','MTChartTip');
    } else {
    ...
    function drawChartTips() {
        ...
        glo.tooltipHandle = divChart.addEventListener('mousemove', (e) => {
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                if (dx * dx + dy * dy <= 64) {
                    const mX = Math.min(mouseX - 48, 400);
                    ...
                    if(ad.includeInNetWorth == true || skipHidden2 == 0) {
                        ...
                        if(MTFlex.Button2 == 1 || MTFlex.Button2 == 3 || useBalance !=0 || begBalance != 0 || rtnIsAccountUsed(MTP.UID,skipHidden3) == true ) {
                            ...
                            if(MTFlex.Button2 != 1 && ad.hideTransactionsFromReports == false) {
                                ...
                                MTFlexRow[MTFlexCR][6] += tI;
                                MTFlexRow[MTFlexCR][7] = +(-tE).toFixed(2);
                                MTFlexRow[MTFlexCR][8] += tT;
                            }
